...
#
# Here the count is not a diagnostic, it is the published version: the patch
# level is `git rev-list --count`, so a shallow clone would name a release that
# already exists with different contents. This script is the first line of that
# defense and deliberately not the only one -- `versionFromGitCount` in the root
# build script refuses outright rather than guessing, and the release workflow
# refuses to overwrite. See README.md, "Consuming it".
#
# Copied from mikelward/mesh, whose sandbox has the same shallow clone. The body
# below is byte-identical to that copy on purpose: it took several review rounds
# to get the bounded-fetch and process-group handling right, so a fix belongs in
# both rather than in a local variant.
set -eu
